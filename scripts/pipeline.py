...
def _unload_ollama_model(generate_url: str, model: str, timeout: int = 20) -> bool:
    """Unload Ollama model from VRAM. Vracia True ak unloadnuté, False inak.

    404 = model neexistuje (bol zmazaný) → ticho preskočiť, žiadny warn.
    """
    try:
        ...
        if response.status_code == 404:
            # Model neexistuje (zmazaný / never installed) — žiadny warn, len skip
            return False
...
def _align_srt_whisperx(
    ...
    try:
        ...
        return
    # Whisperx interne používa pyannote.core (cez diarization). Pre-check že máme dependency.
    try:
        import pyannote.core  # noqa: F401
    except ImportError:
        print("[SRT-ALIGN] pyannote chýba — preskakujem WhisperX SRT alignment.", flush=True)
        print("[SRT-ALIGN] Pre full alignment: pip install pyannote.audio", flush=True)
        print("[SRT-ALIGN] Alebo vypni v GUI Možnosti → 'WhisperX SRT alignment' (uncheck)", flush=True)
        return
...
def main() -> None:
    ...
    # by sa inak preskočili a pipeline by miešala enginy.
    ...
    if not goto_tts and getattr(args, "hybrid_translate", False):
        ...
        _g4_model = getattr(args, "gemma4_hf_model", str(PATHS.gemma4_e4b_hf))
        ...
        # Gemma 3 12B v1 translator (QLoRA fine-tuned, ollama). Rýchlejší + lepšia kvalita ako 26B.
        # content_type=technical/programming/sql → model zachová EN technické termy
        # (engine, shader, framework, rendering...) — inak ich preloží do SK ekvivalentov
        ...
        _g3_v1_batch = int(getattr(args, "g3_v1_batch_size", 6))
        if _g3_v1_batch > 1:
            print(f"[G3-V1-TRANSLATOR] Batch mode: {_g3_v1_batch} segmentov/prompt (anti-halucinácia)", flush=True)
        translated = translate_fulldoc_with_g3_v1(
            segments=segments, tgt_lang=tgt, src_lang=_sw_src_lang,
            content_type=_ct, batch_size=_g3_v1_batch)
        for i, seg in enumerate(translated):
            src_text = (segments[i].get("text") or "").strip()
            seg["text_src"] = src_text
            if _phonetic_on and seg.get("text"):
                seg["text"] = apply_phonetic_respelling(seg["text"])
        # Uvoľni VRAM po preklade — ollama by inak držal model v keep_alive bufferi.
        _g3_url = getattr(args, "ollama_translate_url", "http://localhost:11434/api/generate")
        _unload_ollama_model(_g3_url, "g3-12b-translator-v1:latest", timeout=20)
        ...
        _lms_cli = str(PATHS.lmstudio_bin)
        _llamacpp_server = None  # Will hold LlamaCppServer instance if used

        # Built-in llama-cpp-server alternatíva (ak --use_llamacpp_server alebo LM Studio chýba)
        _use_llamacpp = getattr(args, "use_llamacpp_server", False)
        ...
        if not _ls_running:
            # Voľba 1: built-in llama-cpp-server (open source, súčasť VTS) — ak user chce
            #          alebo LM Studio CLI nie je dostupný
            _lmstudio_available = bool(_lms_cli) and Path(_lms_cli).exists()
            if _use_llamacpp or not _lmstudio_available:
                from llamacpp_server import LlamaCppServer, detect_default_model
                _model_path = getattr(args, "llamacpp_server_model", "") or str(detect_default_model() or "")
                if not _model_path:
                    print("[LLAMACPP] ERROR: Nenašiel som GGUF model (set VTS_LLAMACPP_MODEL alebo --llamacpp_server_model)", flush=True)
                else:
                    _llamacpp_server = LlamaCppServer(
                        model_path=_model_path,
                        port=int(getattr(args, "llamacpp_server_port", 1234)),
                        n_ctx=int(getattr(args, "llamacpp_server_n_ctx", 8192)),
                        n_gpu_layers=int(getattr(args, "llamacpp_server_n_gpu_layers", -1)),
                    )
                    try:
                        _llamacpp_server.start(ready_timeout=180)
                        _ls_url = _llamacpp_server.url
                        _ls_api_url = _ls_url.rstrip("/") + "/models"
                        _ls_running = True
                        print(f"[LLAMACPP] Built-in server beží — pipeline pokračuje na {_ls_url}", flush=True)
                    except Exception as _llcerr:
                        print(f"[LLAMACPP] Server start zlyhal: {_llcerr}", flush=True)
                        _llamacpp_server = None
            # Voľba 2: LM Studio CLI (legacy)
            if not _ls_running and _lmstudio_available:
                import subprocess as _subp, time as _time
                import os as _os
                _lms_env = {**_os.environ, "PATH": f"{Path(PATHS.lmstudio_bin).parent}:{_os.environ.get('PATH','')}"}
        ...
        if _ls_running and _ls_model_key:
            try:
                ...
                if not any(_ls_model_key in _id for _id in _loaded_ids):
                    ...
                    import os as _os2
                    _lms_env2 = {**_os2.environ, "PATH": f"{Path(PATHS.lmstudio_bin).parent}:{_os2.environ.get('PATH','')}"}
        ...
        print(f"[LMSTUDIO-FULLDOC] Full-doc preklad cez LM Studio API ({_ls_url}, model={_ls_model or _ls_model_key})", flush=True)
        translated = translate_fulldoc_with_lmstudio(
            ...
            content_type=content_type,
            src_lang=_sw_src_lang,
            speaker_gender=getattr(args, "speaker_gender", "") or "",
        )
        for i, seg in enumerate(translated):
            src_text = (segments[i].get("text") or "").strip()
            seg["text_src"] = src_text
        ...
        if _compress_enabled and translated:
            ...
            for i, seg in enumerate(translated):
                ...
                if not text:
                    _c_stats["noop"] += 1
                    continue
                # G3 v1 robí oba smery (compress aj expand pri kratšom texte ako target).
                # 26B compress sa volá iba ak je text DLHŠÍ ako target.
                if _use_g3_adj:
            ...
            # Uvoľni G3 length-adjuster z VRAM (drží 9.5 GB v keep_alive bufferi)
            if _use_g3_adj:
                _adj_url = getattr(args, "ollama_translate_url", "http://localhost:11434/api/generate")
                _adj_model = getattr(args, "g3_v1_length_adjuster_model", "g3-12b-length-adjuster-v1")
                _adj_model_with_tag = _adj_model if ":" in _adj_model else f"{_adj_model}:latest"
                _unload_ollama_model(_adj_url, _adj_model_with_tag, timeout=20)

        # Uvoľni 26B model pred TTS — VRAM treba pre OmniVoice / Chatterbox
        if _llamacpp_server is not None:
            # Built-in llama-cpp server — stop subprocess úplne
            try:
                _llamacpp_server.stop()
                print(f"[LLAMACPP] Built-in server zastavený, VRAM uvoľnená.", flush=True)
            except Exception as _stoperr:
                print(f"[LLAMACPP] Stop varování: {_stoperr}", flush=True)
        else:
            # LM Studio external — unload VŠETKY loaded modely (LM Studio si môže auto-loadnúť
            # iný model než _ls_model_key, ak je už loaded; preto querujeme aktuálne loaded ID).
            try:
                import subprocess as _subp_unload, os as _os_unload, requests as _req_unload
                _lms_env_unload = {**_os_unload.environ, "PATH": f"{Path(PATHS.lmstudio_bin).parent}:{_os_unload.environ.get('PATH','')}"}
                _loaded_ids = []
                try:
                    _resp = _req_unload.get(_ls_url.rstrip("/") + "/models", timeout=5).json()
                    _loaded_ids = [m.get("id","") for m in _resp.get("data",[]) if m.get("id")]
                except Exception:
                    pass
                _to_unload = _loaded_ids or [_ls_model_key]
                for _mid in _to_unload:
                    print(f"[LMSTUDIO] Uvoľňujem model {_mid} pred TTS...", flush=True)
                    _subp_unload.run([_lms_cli, "unload", _mid],
                                     env=_lms_env_unload, capture_output=True, timeout=30)
                print(f"[LMSTUDIO] Modely uvoľnené ({len(_to_unload)}).", flush=True)
    ...
    # Pre lmstudio engine preskočíme refine LEN AK by sa robil tým istým modelom (llama_cpp default).
    # Ak user explicitne nastavil refine_model alebo Ollama refine, povolíme — to je 2-LLM pipeline.
    _refine_explicit_model = bool(getattr(args, "refine_model", ""))
    _refine_via_ollama = (
        getattr(args, "use_ollama_adaptation", False)
        or getattr(args, "adapt_provider", "llama_cpp") == "ollama"
    )
    _skip_refine_lmstudio = (
        getattr(args, "use_lmstudio_translate", False)
        and not _refine_explicit_model
        and not _refine_via_ollama
    )
    ...
    # Auto-trigger sk_f keď je speaker_gender=feminine + tgt jazyk je SK/CS, aby refine
    # nemohol prepísať ženské tvary späť na mužské (čo sa diaľšom kontextu stávalo).
    _spk_gender = (getattr(args, "speaker_gender", "") or "").lower()
    if not _gender_fix and _spk_gender == "feminine" and tgt in ("sk", "slk", "cs", "ces"):
        _gender_fix = "sk_f"
        print(f"[GENDER-FIX] Auto-triggered sk_f (speaker_gender=feminine)", flush=True)
    ...
    # POZN.: Pre programming/technical/sql content type skipujeme — tech termíny
    # ako "Metal shader", "noise shader", "DirectX", "OpenGL" zámerne ostávajú v EN
    # bez diakritík, GT by ich neželane prekladal ("noise shader" → "hluk shader").
    _gt_fallback_skip_types = ("programming", "technical", "sql")
    if not goto_tts and tgt in ("slk", "sk") and translated \
            and content_type not in _gt_fallback_skip_types:
        ...
        for _seg in translated:
            ...
            if len(_txt) > 8 and not any(c in _SK_CHARS for c in _txt):
                try:
                    _fixed = translate_segment_with_google(
                        _txt, tgt, src_lang="en",
                        apply_phonetic=_phonetic_on,  # respektuj --no-phonetic_respelling
                    )
    ...
    elif content_type in _gt_fallback_skip_types and translated:
        print(f"[FALLBACK-GT] Skipnuté pre content_type={content_type} "
              f"(tech termíny ostávajú v EN zámerne)", flush=True)
        # SQL cleanup IBA pre content_type=sql (rule "nuly → NULL hodnoty" je príliš
        # agresívna pre programming context — "od nuly" = "from scratch", nie SQL NULL).
        if _sql_tts_on:
            translated = apply_sql_source_guided_cleanup(translated)
        if _mixed_technical_terms and content_type == "technical":
            translated = apply_linux_source_guided_cleanup(translated, mixed_mode=True)
    ...
    if not goto_tts and getattr(args, "use_mini_level11", False) and translated:
        if tgt in ("sk", "slk"):
            try:
                ...
                # Post mini-L11 tech-term cleanup: G3-12B fine-tune občas prepíše
                # "engine" → "motor" pri rewrite. Aplikujeme deterministický fix
                # rovnako ako po LM Studio translate.
                from translation import _post_translate_fix_tech_terms as _post_fix
                _mini_fixed = 0
                for _seg_m in translated:
                    _t_in = _seg_m.get("text", "")
                    if not _t_in:
                        continue
                    _src_en = _seg_m.get("text_src", "") or ""
                    _t_out = _post_fix(_t_in, src_text=_src_en)
                    if _t_out != _t_in:
                        _seg_m["text"] = _t_out
                        _mini_fixed += 1
                if _mini_fixed:
                    print(f"[MINI-L11] Tech-term cleanup po mini_level11: {_mini_fixed} segmentov opravených (motor→engine).", flush=True)
    ...
    if getattr(args, "use_g3_27b_translator", False):
        _g3_27b_model = getattr(args, "g3_27b_translator_model", "g3-27b-multitask-v1")
    ...
    # 4) Cascade fallback model — 26b-translator-q6 odstránený 2026-05-06 (broken),
    # ponechaný len ak by si ho user vrátil (ollama pull). Bezpečné — _unload_ollama_model
    # toleruje 404 (model neexistuje), len log warn.
    ...
    if use_s2:
        ...
        print(f"[OMNIVOICE] Using OmniVoice (zero-shot): {getattr(args, 'omnivoice_repo', 'k2-fsa/OmniVoice')}", flush=True)
    elif use_cb:
        print(f"[CHATTERBOX] Using Chatterbox TTS with model: {args.chatterbox_model}", flush=True)

    # STOP-AFTER-TRANSLATE — pipeline má zakázané zasahovať do audia.
    # Save segments JSON a exit. Audio sa robí samostatne (cez GUI orchestrate alebo CLI).
    # FORCE pre OmniVoice — pipeline orchestrate nepokrýva celý chain (clone + batch +
    # dynamic align + master), takže ho prenecháme GUI / standalone skriptom.
    _force_stop = use_omnivoice
    if _force_stop and not getattr(args, "stop_after_translate", False):
        print("[OMNIVOICE] AUTO-STOP: pipeline nedotkne audia, GUI/standalone urobí celý chain.", flush=True)
    if getattr(args, "stop_after_translate", False) or _force_stop:
        # Pre OmniVoice/stop-after-translate path beží `apply_postfix` až v TTS adaptation,
        # ktorá je ZA stop. Aplikujeme ju TU aby sa fixed text dostal do segments JSON.
        # Catches: "NULL hodnoty" → "nuly", "Určite sa pýtate" → "Možno sa pýtate", atď.
        if tgt in ("slk", "sk") and translated:
            try:
                from sk_glossary import apply_postfix as _apply_postfix
                _postfix_count = 0
                for _seg in translated:
                    _txt = (_seg.get("text") or "").strip()
                    if not _txt:
                        continue
                    _fixed, _hits = _apply_postfix(_txt)
                    if _fixed != _txt:
                        _seg["text"] = _fixed
                        _postfix_count += 1
                        print(f"[POSTFIX] {_txt[:60]} → {_fixed[:60]}", flush=True)
                if _postfix_count:
                    print(f"[POSTFIX] Aplikovaný na {_postfix_count} segmentov pred STOP", flush=True)
            except Exception as _pe:
                print(f"[POSTFIX] Warning: {_pe}", flush=True)

        _stop_seg_json = out_dir / f"{stem}_{tgt}_segments.json"
        try:
            _write_segments_json(translated, _stop_seg_json)
        except Exception as _se:
            print(f"[STOP] Save segments warning: {_se}", flush=True)
        print(f"\n[STOP] Translate dokončený. Pipeline NEZASIAHNE do audia.", flush=True)
        print(f"[STOP] Segments JSON: {_stop_seg_json}", flush=True)
        print(f"[STOP] Pre TTS audio spusti:", flush=True)
        print(f"       cd {Path(__file__).parent.parent}", flush=True)
        print(f"       ./test_omnivoice_batch.py --segments '{_stop_seg_json}'", flush=True)
        sys.exit(0)

    # POZN.: Pipeline orchestrate pre OmniVoice bol odstránený — auto-stop vyššie
    # zaisťuje že GUI / standalone urobia celý chain (clone + batch + align + master).
    # Pipeline iba do translate.
    if False and use_omnivoice:
        print(f"\n[OMNIVOICE] Pipeline urobil preklad → posielam segments OmniVoice TTS.", flush=True)
        _seg_json = out_dir / f"{stem}_{tgt}_segments.json"
        try:
            _write_segments_json(translated, _seg_json)
        except Exception as _se:
            print(f"[OMNIVOICE] Save segments warning: {_se}", flush=True)
        # Standalone skript v repo root
        _repo_root = Path(__file__).resolve().parent.parent
        _ov_script = _repo_root / "test_omnivoice_batch.py"
        _ov_clone_script = _repo_root / "auto_clone_ref.py"
        if not _ov_script.exists():
            print(f"[OMNIVOICE] CHYBA: standalone skript {_ov_script} neexistuje.", flush=True)
            sys.exit(1)
        # Output WAV — pipeline expected location pre mux
        _ov_out_wav = out_dir / f"{stem}_{tgt}_tts.wav"
        # Ref audio: ak je v args explicitné, použi to; inak auto-clone z input videa
        _ov_ref = getattr(args, "omnivoice_ref_audio", "") or ""
        # Validate: ak je ref invalidná cesta (napr. sentinel "🎬 Cloning z videa")
        # alebo neexistuje, nechaj ho prázdny → auto-clone trigger nižšie
        if _ov_ref and not Path(_ov_ref).exists():
            print(f"[OMNIVOICE] Ref audio path neexistuje ({_ov_ref}) → auto-clone z videa", flush=True)
            _ov_ref = ""
        if not _ov_ref and _ov_clone_script.exists():
            # Auto-clone z input videa
            print(f"[OMNIVOICE] Auto-clone ref z input videa: {Path(args.input).name}", flush=True)
            _clone_target = _repo_root / "voices" / f"{Path(args.input).stem}_clone.wav"
            _clone_cmd = [
                sys.executable, str(_ov_clone_script), str(args.input),
                "--out", str(_clone_target),
                "--ref-lang", "en",  # source language, Whisper transkribuje EN ref
            ]
            import subprocess as _ov_subp_clone
            _clone_proc = _ov_subp_clone.run(_clone_cmd, timeout=600)
            if _clone_proc.returncode == 0 and _clone_target.exists():
                _ov_ref = str(_clone_target)
                print(f"[OMNIVOICE] Auto-clone OK: {_clone_target}", flush=True)
            else:
                print(f"[OMNIVOICE] Auto-clone zlyhal → fallback greeting", flush=True)
                _ov_ref = str(_repo_root / "voices" / "chatterbox_sk_greeting_studio.wav")
        if not _ov_ref:
            _ov_ref = str(_repo_root / "voices" / "chatterbox_sk_greeting_studio.wav")
        _ov_cmd = [
            sys.executable, str(_ov_script),
            "--segments", str(_seg_json),
            "--out", str(_ov_out_wav),
            "--ref", _ov_ref,
            "--num-step", str(int(getattr(args, "omnivoice_num_step", 64))),
            "--guidance", f"{float(getattr(args, 'omnivoice_guidance', 2.5)):.2f}",
            "--lang", getattr(args, "omnivoice_language", "sk"),
        ]
        print(f"[OMNIVOICE] $ {' '.join(_ov_cmd[:3])} ...", flush=True)
        import subprocess as _ov_subp
        _ov_proc = _ov_subp.run(_ov_cmd, timeout=3600)
        if _ov_proc.returncode != 0:
            print(f"[OMNIVOICE] Standalone batch zlyhal rc={_ov_proc.returncode}", flush=True)
            sys.exit(1)
        if not _ov_out_wav.exists():
            print(f"[OMNIVOICE] Audio neviem nájsť: {_ov_out_wav}", flush=True)
            sys.exit(1)
        print(f"[OMNIVOICE] Audio hotové: {_ov_out_wav}", flush=True)
        # Skočí ku final mux/post-process — pipeline pokračuje so samostatne vyrobeným audiom
        # Pozn: ďalej nasleduje normálny pipeline flow ktorý ide od TTS phase. Aby to fungovalo,
        # nastavíme premenné tak, aby ďalšie kroky skipli (audio už existuje).
        # SIMPLEST: exit teraz, GUI urobí mux. Pre CLI usage user musí muxnut sám.
        print(f"[OMNIVOICE] Pipeline END. Audio v: {_ov_out_wav}", flush=True)
        sys.exit(0)
    elif args.use_xtts:
    ...
    for i, seg in enumerate(translated):
        ...
        if use_turbo and not _turbo_route_reasons:
            ...
            # OmniVoice je diffusion model — krátke texty (<12 znakov) často halucinuje silence/hum.
            # Pre takéto použijeme Chatterbox directly — rýchlejšie aj kvalitnejšie pre krátke vety.
            _omnivoice_min_chars = getattr(args, "omnivoice_min_chars", 12)
            _text_clean = (text or "").strip()
            if len(_text_clean) < _omnivoice_min_chars:
                print(f"[OMNIVOICE] Skip (text len {len(_text_clean)}<{_omnivoice_min_chars}) → Chatterbox fallback: {_text_clean!r}", flush=True)
                # chatterbox_tts je importovaný na vrchu pipeline.py — žiadny local re-import.
                # Použijeme args.chatterbox_* priamo (lokálne _base_* sú definované až v use_cb vetve).
                _ov_fb_exag = float(getattr(args, "chatterbox_exaggeration", -1.0))
                _ov_fb_cfg = float(getattr(args, "chatterbox_cfg_weight", -1.0))
                _ov_fb_temp = float(getattr(args, "chatterbox_temperature", -1.0))
                # Sane defaults ak args nemá explicit hodnoty (-1.0 = use Chatterbox internal default)
                if _ov_fb_exag < 0: _ov_fb_exag = 0.5
                if _ov_fb_cfg < 0: _ov_fb_cfg = 0.5
                if _ov_fb_temp < 0: _ov_fb_temp = 0.6
                chatterbox_tts(
                    text=text,
                    output_path=out_chunk,
                    model_path=args.chatterbox_model,
                    device=device,
                    use_torch_compile=use_torch_compile,
                    audio_prompt_path=audio_prompt,
                    safe_mode=getattr(args, "tts_safe_mode", False),
                    ultra_safe=getattr(args, "tts_ultra_safe", False),
                    exaggeration=_ov_fb_exag,
                    cfg_weight=_ov_fb_cfg,
                    temperature=_ov_fb_temp,
                )
            else:
        ...
        if out_chunk.exists():
            ...
            if _is_bad and _used_cb_for_segment:
                ...
                if _is_bad:
                    ...
                    write_silence_chunk(out_chunk, _seg_dur)
                    _bad_seg_log.append({"seg": i+1, "reason": _reason, "slot": _seg_dur, "text": text})
            elif _is_bad and use_omnivoice:
                # OmniVoice halucinuje (looping, krátky output, distortion). Skús:
                # 1) OmniVoice retry s nižším guidance (stabilnejší, menej halucinuje)
                # 2) Chatterbox fallback (iná farba hlasu, ale slová zachované — lepšie ako ticho)
                # 3) Silence iba ak ani 2) nepomôže
                print(f"[QA] Seg {i+1} OmniVoice BAD ({_reason}) — retry s nižším guidance", flush=True)
                try:
                    from tts import omnivoice_tts as _ov_retry
                    _ov_retry(
                        text=text, output_path=out_chunk,
                        repo_or_path=getattr(args, "omnivoice_repo", None),
                        ref_audio=getattr(args, "omnivoice_ref_audio", None) or voice_sample,
                        ...
                        language=getattr(args, "omnivoice_language", "sk"),
                        num_step=getattr(args, "omnivoice_num_step", 64),
                        guidance_scale=1.5,  # nižšie ako default 2.5 — stabilnejšie
                        instruct=getattr(args, "omnivoice_instruct", "") or "",
                        speed=getattr(args, "omnivoice_speed", 1.0),
                        expressive=False,  # vypnuté pre stabilitu
                    )
                except Exception as _ov_retry_err:
                    print(f"[QA] OmniVoice retry zlyhal: {_ov_retry_err}", flush=True)
                _is_bad, _reason = inspect_tts_chunk(out_chunk, expected_sec=_seg_dur)
                if _is_bad:
                    print(f"[QA] Seg {i+1} OmniVoice retry stále BAD ({_reason}) — Chatterbox fallback (iný hlas, ale slová zachované)", flush=True)
                    try:
                        chatterbox_tts(
                            text=text, output_path=out_chunk,
                            model_path=args.chatterbox_model,
                        ...
                        print(f"[QA] Chatterbox fallback zlyhal: {_cb_fb_err}", flush=True)
                if _is_bad:
                    print(f"[QA] Seg {i+1} fallback chains exhausted — silence", flush=True)
                    write_silence_chunk(out_chunk, _seg_dur)
                    _bad_seg_log.append({"seg": i+1, "reason": _reason, "slot": _seg_dur, "text": text})
            elif _is_bad:
                # Non-Chatterbox / non-OmniVoice TTS: no retry, just replace with silence
            ...
            if out_chunk.exists():
                ...
                # silence_repair iba preskočený pre OmniVoice (postprocess_output=False
                # už zaisťuje že nemá dlhé internal pauses a naša detection cuts tichú
                # samohláskovú konvolvolúciu). Chatterbox má pôvodné správanie.
        ...
        if out_chunk.exists():
            ...
            # Helper — re-render segmentu rovnakým primary engine ako základné TTS.
            # KRITICKÉ: pri OmniVoice primary nepoužiť Chatterbox na re-TTS, lebo
            # Chatterbox má iný hlas → počuteľná zmena timbre v 1 sekunde segmentu.
            def _re_render_with_primary(_text_in: str, _out_path):
                _t = (_text_in or "").strip()
                _ov_min = int(getattr(args, "omnivoice_min_chars", 12))
                if use_omnivoice and len(_t) >= _ov_min:
                    from tts import omnivoice_tts as _ov_re
                    _ov_re(
                        text=_text_in, output_path=_out_path,
                        repo_or_path=getattr(args, "omnivoice_repo", None),
                        ref_audio=getattr(args, "omnivoice_ref_audio", None) or voice_sample,
                        ref_text=getattr(args, "omnivoice_ref_text", None),
                        omnivoice_python=getattr(args, "omnivoice_python", None),
                    ...
                    return "omnivoice"
                if use_f5:
                    from tts import f5_tts as _f5_re
                    _f5_re(
                        text=_text_in, output_path=_out_path,
                        ckpt_path=getattr(args, "f5_model", None),
                        ref_audio=getattr(args, "f5_ref_audio", None) or voice_sample,
                    ...
                    return "f5"
                # Chatterbox fallback (krátky text v OmniVoice režime, alebo Chatterbox primary)
                _exag = float(getattr(args, "chatterbox_exaggeration", -1.0))
                _cfg  = float(getattr(args, "chatterbox_cfg_weight", -1.0))
                _temp = float(getattr(args, "chatterbox_temperature", -1.0))
                if _exag < 0: _exag = 0.5
                if _cfg < 0: _cfg = 0.5
                if _temp < 0: _temp = 0.6
                chatterbox_tts(
                    text=_text_in, output_path=_out_path,
                    model_path=args.chatterbox_model,
                    device=device,
            ...
            if _timing_retry_enabled and not use_s2 and _dp_action == _DPAction.REWRITE_SHRINK:
                ...
                if _shrink_ollama_url and _shrink_ollama_model:
                    ...
                    if _shrunk_text and len(_shrunk_text) < len(text):
                        ...
                        try:
                            ...
                            _engine_used = _re_render_with_primary(_shrink_render, out_chunk)
                            print(f"[PLANNER] Seg {i+1} shrink re-TTS engine={_engine_used}", flush=True)
                ...
                # Bumpnutý limit z 1.18 na 1.50 — pri OmniVoice re-TTS produkuje dlhšie audio
                # než Chatterbox a 1.18× nestačí pokryť prebytok → segment ostáva too_long →
                # assembly atempo zachytí neskôr (znelo to ako "z ničoho nič zrýchli audio").
                # 1.50× je počuteľne rýchlejšie ale komprehensibilné, lepšie ako neskôr v assembly.
                if not _shrink_accepted or _dp_action == _DPAction.REWRITE_SHRINK:
                    _atempo_shrink = round(min(1.50, max(1.0, _dp_guard.stretch_ratio)), 4)
                    print(
            ...
            elif _timing_retry_enabled and use_s2 and _dp_action == _DPAction.REWRITE_SHRINK:
            ...
            # REWRITE_EXPAND → ollama expand + re-TTS, fallback na apad
            elif _timing_retry_enabled and _dp_action == _DPAction.REWRITE_EXPAND:
                ...
                if _expanded and _expanded != text and len(_expanded) > len(text) * 0.95:
                    ...
                    # Re-render s primary engine (OmniVoice ak bol primary, Chatterbox inak).
                    # KRITICKÉ: nepoužiť Chatterbox keď OmniVoice bol primary — iný hlas.
                    try:
                        _engine_used = _re_render_with_primary(_expanded, out_chunk)
                        print(f"[PLANNER] Seg {i+1} expand re-TTS engine={_engine_used}", flush=True)
            ...
            # SPLIT_OR_RETRY — last resort: trim (too_long) alebo apad (too_short/low_fill)
                ...
                if _sor_issue == "too_long":
                    # Najprv skús AGRESÍVNE atempo (až do 1.8×) — zachová slová, len rýchlejšie.
                    # Hard trim by orezal slová v polovici → rušivé. Atempo do 1.8× je počuteľné
                    # ale komprehensibilné. Iba ak ani 1.8× nestačí (audio >1.8× slot), trim.
                    _ratio = _dp_guard.wav_dur / _slot_dur if _slot_dur > 0 else 1.0
                    _MAX_ATEMPO_HARD = 1.80
                    _atempo_used = min(_ratio, _MAX_ATEMPO_HARD)
                    _atempo_filter = f"atempo={_atempo_used:.4f}"
                    if _atempo_used > 2.0:
                        # ffmpeg atempo per-call cap = 2.0; chain dvoch atempo
                        _half = _atempo_used ** 0.5
                        _atempo_filter = f"atempo={_half:.4f},atempo={_half:.4f}"

                    _stretched = out_chunk.with_suffix(".sor_stretched.wav")
                    _stretch_rc = _sp.run([
                        "ffmpeg", "-y", "-i", str(out_chunk),
                        "-af", _atempo_filter,
                        str(_stretched),
                    ...
                    _atempo_ok = _stretch_rc.returncode == 0 and _stretched.exists()
                    if _atempo_ok:
                        shutil.move(str(_stretched), str(out_chunk))
                        _dp_guard = _dp_eval(out_chunk, _slot_dur, _dp_spec)
                        _timing_entry = _make_timing_entry(text)
                        _sor_action = f"atempo_aggressive={_atempo_used:.2f}"
                        print(
                            f"[PLANNER] Seg {i+1} too_long zachránené atempo={_atempo_used:.2f}× "
                            f"(slot={_slot_dur:.2f}s, wav_before={_ratio*_slot_dur:.2f}s)",
                            flush=True,
                        )
                    else:
                        _stretched.unlink(missing_ok=True)

                    # Hard trim ako absolútne posledná možnosť — iba ak audio JE EŠTE
                    # delšie než slot AJ po max atempo (slot je extrémne krátky vs text)
                    if not _atempo_ok or _dp_guard.wav_dur > _slot_dur + 0.10:
                        ...
                        if _trim_rc.returncode == 0 and _trimmed.exists():
                            ...
                            _dp_guard = _dp_eval(out_chunk, _slot_dur, _dp_spec)
                            _timing_entry = _make_timing_entry(text)
                            _sor_action = ("atempo_then_hard_trim" if _atempo_ok else "hard_trim")
                            print(
                                f"[PLANNER] Seg {i+1} hard_trim FALLBACK — audio stále príliš dlhé",
                                flush=True,
                            )
                        else:
                            _trimmed.unlink(missing_ok=True)
    ...
    # --- Post-TTS EQ polish (highs softening / EQ profile) ---
    _eq_profile = getattr(args, "tts_eq_profile", "") or ""
    _soften_highs = getattr(args, "soften_highs", False) or _eq_profile == "Soft Highs"
    if _eq_profile and _eq_profile != "Soft Highs":
        from config import TTS_EQ_PROFILES
        from audio_pipeline import ffmpeg_apply_filter
        if _eq_profile in TTS_EQ_PROFILES:
            af = ",".join(TTS_EQ_PROFILES[_eq_profile])
            _eq_tmp = out_wav.with_name(f"{out_wav.stem}_eq.wav")
            if ffmpeg_apply_filter(out_wav, _eq_tmp, af):
                shutil.move(str(_eq_tmp), str(out_wav))
                print(f"[EQ] Applied profile '{_eq_profile}': {af}", flush=True)
    if _soften_highs:
        from audio_pipeline import apply_soft_high_cut
        if apply_soft_high_cut(out_wav):
            print("[EQ] Soft high-cut applied (lowpass 7800Hz + 3 narrow notches)", flush=True)
