...
TTS_EQ_PROFILES: dict = {
    "Soft Highs": [
        # Pre OmniVoice / TTS engines s príliš ostrými výškami + tenkým hlasom.
        # Aggressive high-cut + bass boost pre warmth.
        "lowpass=f=6500",                          # tvrdý orez nad 6.5 kHz
        "equalizer=f=2800:t=q:w=1.0:g=-2.5",       # tlmí presence
        "equalizer=f=4000:t=q:w=1.0:g=-3.5",       # tlmí harshness
        "equalizer=f=5500:t=q:w=1.0:g=-4.0",       # tlmí sibilance
        # Bass / low-mid boost — viac warmth a "telo" hlasu
        "highpass=f=70",                           # orez subsonic
        "equalizer=f=110:t=q:w=0.9:g=+3.5",        # boost basov
        "equalizer=f=180:t=q:w=1.0:g=+2.0",        # boost low-mids (warmth)
        "equalizer=f=350:t=q:w=1.0:g=+1.0",        # mild fullness
    ],
    "Hegen": [
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--llama_model", default=str(PATHS.gemma4_26b_base_q4km))
    ...
    ap.add_argument("--phonetic_respelling", action=argparse.BooleanOptionalAction, default=None,
                    help="Enable/disable phonetic respelling map. Default: ON for general content, OFF for production_dub preset. "
                         "Use --no-phonetic_respelling to force off.")
    ...
    ap.add_argument("--gemma4_hf_model", default=str(PATHS.gemma4_e4b_hf), help="Path to Gemma 4 E4B model directory (HuggingFace format).")
    ...
    ap.add_argument("--soften_highs", action="store_true", help="Aplikuj soft high-cut (lowpass 7800Hz + tame 3.4/5.2/6.8 kHz) na finálne TTS audio. Užitočné pre OmniVoice keď znie príliš ostro.")
    ...
    # F5-TTS — opt-in, defaulty prázdne (vyžaduje user-side cesty cez env / CLI)
    ap.add_argument("--f5_model", default=os.environ.get("VTS_F5_CKPT", ""), help="Path to F5-TTS SK fine-tuned ckpt (.pt). Set VTS_F5_CKPT or pass explicitly.")
    ap.add_argument("--f5_ref_audio", default=os.environ.get("VTS_F5_REF_AUDIO", ""), help="F5-TTS SK reference audio")
    ap.add_argument("--f5_ref_text", default="", help="Transcript of f5_ref_audio (auto-Whisper if empty)")
    ap.add_argument("--f5_python", default=os.environ.get("VTS_F5_PYTHON", ""), help="Path to f5tts_env python")

    # OmniVoice TTS (k2-fsa, Apache 2.0, 600+ jazykov zero-shot, primary engine od 2026-05-05)
    ap.add_argument("--use_omnivoice", action="store_true", help="Use OmniVoice TTS (zero-shot multilingual, najvyšší SK score)")
    ap.add_argument("--omnivoice_repo", default="k2-fsa/OmniVoice", help="OmniVoice HF repo alebo local path")
    ap.add_argument("--omnivoice_ref_audio", default="", help="Reference audio (cleaned cez cleanup_reference_audio). Empty = auto-pick z voices/")
    ap.add_argument("--omnivoice_ref_text", default="", help="Transcript of ref_audio (empty = auto Whisper transcribe)")
    ...
    ap.add_argument("--omnivoice_python", default="", help="Path to env with omnivoice package (empty = auto-detect cez paths.PATHS.python_fish)")
    ap.add_argument("--omnivoice_instruct", default="", help="Voice design tags (napr. 'male, young adult, moderate pitch')")
    ap.add_argument("--omnivoice_speed", type=float, default=1.0, help="Speech speed multiplier (1.0=normal)")
    ...
    # Built-in alternative to LM Studio — open-source llama-cpp-python server
    ap.add_argument("--use_llamacpp_server", action="store_true",
                    help="Auto-spustí llama-cpp-python OpenAI-compatible server namiesto LM Studio. "
                         "Použije sa rovnaký endpoint (localhost:1234/v1) — pipeline-u nezmení.")
    ap.add_argument("--llamacpp_server_model", default="",
                    help="GGUF model path pre llama-cpp-server. Empty = auto-detect (paths.gemma4_26b_base_q4km).")
    ap.add_argument("--llamacpp_server_port", type=int, default=1234,
                    help="Port pre llama-cpp-server (default 1234, kompatibilný s LM Studio)")
    ap.add_argument("--llamacpp_server_n_ctx", type=int, default=8192,
                    help="Context size pre llama-cpp-server (default 8192)")
    ap.add_argument("--llamacpp_server_n_gpu_layers", type=int, default=-1,
    ...
    ap.add_argument("--sk_corrector_model", default="", help="Ollama model name for SK grammar corrector pass. Default: empty (off — sk-corrector-v* boli halucinujúce, použi --refine_translation s g3-12b-translator-v1 namiesto)")
    ...
    ap.add_argument("--finetuned_translator_model", default="g3-12b-translator-v1", help="Ollama model name for fine-tuned translator (default: g3-12b-translator-v1 — overený stabilný 2026-05-06)")
    ...
    ap.add_argument("--g3_v1_batch_size", type=int, default=6, help="Batch size pre G3-12B translator (default 6). 0 = legacy per-segment (halucinuje na krátkych vetách). Vyššia hodnota dáva model viac kontextu = menej halucinácií.")
    ...
    ap.add_argument(
        "--adapt_ollama_model",
        default="g3-12b-translator-v1",
        help="Ollama model used for timing-aware text adaptation. Default g3-12b-translator-v1 (po cleanup-e 2026-05-06, sk-gemma4-e4b-v6 odstránený).",
    )
    ap.add_argument(
    ...
    # Stop pipeline after translate (no TTS, no audio mixing). Output: segments JSON
    # ready for standalone TTS testing (test_omnivoice_batch.py).
    # Pipeline má prísne zakázané zasahovať do audia v tomto režime.
    ap.add_argument(
        "--stop_after_translate", action="store_true",
        help="Pipeline urobí iba STT+translate, segmenty uloží do JSON a exit pred TTS. "
             "Audio si vyrobíš ty sám cez ./test_omnivoice_batch.py.",
    )
    # Speaker gender hint pre translate — ovplyvní 1. osobu jed. č. minulého času
    # ("Začal som" vs "Začala som"). Default masculine ak nie je špecifikované.
    ap.add_argument(
        "--speaker_gender", default="", choices=["", "masculine", "feminine"],
        help="Pohlavie hovoriacej osoby pre preklad: 'feminine' = ženské tvary "
             "slovies v 1.os. sg. minulého času. Default: prázdne (masculine).",
    )
    return ap
