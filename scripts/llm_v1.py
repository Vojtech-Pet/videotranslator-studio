...
# Adjuster (length pass) môže ísť na separátnu ollama instance (CPU-only)
# aby neblokovala GPU pri sčastných behoch. Default = rovnaká ako OLLAMA_URL.
import os as _os
ADJUSTER_OLLAMA_URL = _os.environ.get("ADJUSTER_OLLAMA_URL", OLLAMA_URL)
TRANSLATOR_MODEL = "g3-12b-translator-v1"
ADJUSTER_MODEL = "g3-27b-multitask-v2"  # 2026-05-03: upgrade z 12B → 27B (multitask translate+length, lepšia kvalita)
...
def _ollama_generate(model: str, prompt: str, num_predict: int = 512,
    ...
    url: defaultne OLLAMA_URL (port 11434), pre adjuster použiť ADJUSTER_OLLAMA_URL.
    """
    body = json.dumps({
        "model": model,
        "prompt": prompt,
...
def adjust_length_g3_v1(sk_text: str, target_chars: int, *,
    ...
    raw = _ollama_generate(ADJUSTER_MODEL, prompt,
                            ...
                            timeout=300,  # CPU inference je pomalšia
                            url=ADJUSTER_OLLAMA_URL)
    if raw.startswith("[ERROR"):
        return sk_text
    cleaned = _strip_format_artifacts(raw)
