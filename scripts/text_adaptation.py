...
def _build_prompt_slot_aware(
    ...
    mode_hint = {
        "balanced": "Jemne uhladť vetu a skráť ju len mierne.",
        "dub_friendly": "Sprav vetu kratšiu, hovorenú a rytmicky vhodnú pre dabing.",
        "aggressive": "Maximálne skráť vetu pri zachovaní jadra významu a technických termínov.",
    ...
    system = (
        ...
        "- Výstup musí byť hovorený, stručný a prirodzený.\n"
        "- Výstup musí byť kratší, ak je pôvodný text na slot pridlhý.\n"
        "- Nepoužívaj doslovné anglické konštrukcie.\n"
        f"- Cieľová dĺžka: najviac {target_chars} znakov.\n"
...
def adapt_for_timing(
    ...
    if rewrite_mode == "balanced" and ratio < threshold_hard:
        attempts = [
            ...
            ("aggressive", max(10, int(target_chars * 0.88))),
        ]
    elif rewrite_mode == "dub_friendly":
        attempts = [
            ("dub_friendly", target_chars),
            ("aggressive", max(10, int(target_chars * 0.90))),
        ]
    else:
        attempts = [
            ("aggressive", target_chars),
            ("aggressive", max(10, int(target_chars * 0.85))),
        ]
    ...
    for mode_name, mode_target_chars in attempts:
        ...
        if (new_ratio < best_ratio) or (abs(new_ratio - best_ratio) < 1e-6 and new_cps < best_cps):
            best_text = rewritten
            best_est = new_est
